...
function OnboardingPage() {
  ...
  React.useEffect(() => {
    ...
    setUsername((p) => p || onboardingState.username || '')
    setTitle((p) => p || onboardingState.title || '')
    setDisplayName((p) => p || onboardingState.name || '')
    setBio((p) => p || onboardingState.bio || '')
    setAvatarUrl((p) => p || onboardingState.image || '')
  }, [onboardingState])
  ...
  React.useEffect(() => {
    ...
    navigate({ search: { page: firstIncompletePage }, replace: true })
  }, [currentPageIndex, firstIncompletePage, maxAllowedPageIndex, navigate])
